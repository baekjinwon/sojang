...
$od_out_number    = clean_xss_tags($od_out_number);
...
$od_out_number    =  isset($_POST['od_out_number']) ? clean_xss_tags($_POST['od_out_number']) : '';
...
$od2 = sql_fetch(" select * from {$g5['g5_shop_order_table']} where od_id = '$od_id' ");
$odct2 = sql_fetch(" select * from g5_shop_cart where od_id = '$od_id' limit 1");
...
if($od_settle_case == '무통장' && $default['de_sms_use2'] && $od_misu > 0) {
    $messages = array(
        array(
            "to" => preg_replace("/[^0-9]/", "", $od_tel),
            "from" => "07087772705",
            "kakaoOptions" => array(
                "pfId" => "KA01PF220420092241845yWZJ1Rxczsl",
                "templateId" => "KA01TP220422160720194f3iZp8PCZ0n",
                "variables" => array(
                    "#{고객명}" => $od_name,
                    "#{상품명}" => $odct2['it_name'],
                    "#{일시}" => G5_TIME_YMDHIS
    ...
    $messages2 = array(
        array(
            "to" => preg_replace("/[^0-9]/", "", $default['de_sms_hp']),
            "from" => "07087772705",
            "kakaoOptions" => array(
                "pfId" => "KA01PF220420092241845yWZJ1Rxczsl",
                "templateId" => "KA01TP220422160815625N29S669JlBs",
                "variables" => array(
                    "#{고객명}" => $od_name,
                    "#{일시}" => $od2['od_time'],
                    "#{상품명}" => $odct2['it_name'],
                    "#{금액}" => number_format($tot_ct_price + $od_send_cost + $od_send_cost2),
                    "#{연락처}" => $od_phone
...
if($config['cf_sms_use'] && ($default['de_sms_use2'] || $default['de_sms_use3'])) {
    ...
    if($is_sms_send) {
        ...
        if($od_settle_case == '무통장' && $default['de_sms_use2'] && $od_misu > 0) {
            ...
            $recv_number = preg_replace("/[^0-9]/", "", $od_tel);
