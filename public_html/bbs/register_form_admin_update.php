...
// 이미지업로드
$file_url = '';
if($_FILES['mb_zip_file']['tmp_name']){
    $file_url = $mb_id.'_'.$_FILES['mb_zip_file']['name'];
    $it_img1 = upload_file($_FILES['mb_zip_file']['tmp_name'], $mb_id.'_'.$_FILES['mb_zip_file']['name'], G5_DATA_PATH.'/member');
}
...
if ($config_damong['cf_kakao_alert_use'] && $config_damong['cf_kakao_alert_temp1'])
{
        //카카오알림
$receive_numbersms = preg_replace("/[^0-9]/", "", $mb_tel);  // 수신자번호 (회원님의 핸드폰번호)

$messages = array(
    // 변수가 있는 경우
    array(
        "to" => $receive_numbersms,
        "from" => $from_kakao_number,
        "kakaoOptions" => array(
            "pfId" => $config_damong['cf_kakao_pfid'],
            "templateId" => $config_damong['cf_kakao_alert_temp1'],
            "variables" => array(
                "#{고객명}" => $mb_name
            )
        )
...
if ($w == '') {
    ...
    alert('관리자 승인이후에 회원가입이 완료됩니다. 감사합니다.', G5_URL);
