...
//카카오알림
require_once(G5_LIB_PATH."/message.php");
check_admin_token();
...
for ($i=0; $i<$count_post_chk; $i++)
{
    ...
    $config_damong = sql_fetch("select * from g5_config");
    if ($change_status == '배송'){
       
        
        //카카오알림
        if($config_damong['cf_kakao_alert_use'] && $config_damong['cf_kakao_alert_temp5']){
            $receive_number = preg_replace("/[^0-9]/", "", $od['od_tel']);	// 수신자번호
            $from_kakao_number = preg_replace("/[^0-9]/", "", $config_damong['cf_kakao_from_number']);
            $messages = array(
                // 변수가 있는 경우
                array(
                    "to" => $receive_number,
                    "from" => $from_kakao_number,
                    "kakaoOptions" => array(
                        "pfId" => $config_damong['cf_kakao_pfid'],
                        "templateId" => $config_damong['cf_kakao_alert_temp5'],
                        "variables" => array(
                            "#{회사명}" => $default['de_admin_company_name'],
                            "#{이름}" => $od['od_name'],
                            "#{택배회사}" => $delivery_company,
                            "#{운송장번호}" => $invoice
                        )
                    )
                )
    ...
    switch ($current_status)
    {
        case '주문' :
            ...
            change_status($od_id, '주문', '입금');  //입금상태로 만들고,  order_update_receipt 정상작동 시킨다.
            order_update_receipt($od_id);

            change_status($od_id, '입금', '완료');  //다시 완료상태로 변경처리
        ...
        case '입금' :
            ...
            $receive_number = preg_replace("/[^0-9]/", "", $od['od_tel']);	// 수신자번호 (받는사람 핸드폰번호 ... 여기서는 주문자님의 핸드폰번호임)
            ...
            $messages = array(
                array(
                    ...
                    "kakaoOptions" => array(
                        "pfId" => $config_damong['cf_kakao_pfid'],
                        "templateId" => $config_damong['cf_kakao_alert_temp4'],
                        "variables" => array(
                            "#{회사명}" => $default['de_admin_company_name'],
                            "#{이름}" => $od['od_name'],
                            "#{입금액}" => number_format($od_receipt_price),
                            "#{주문번호}" => $od['od_id']
...
$return_url = G5_ADMIN_URL . "/?dir=shop&amp;pid=orderlist&amp;$qstr";
goto_url($return_url);