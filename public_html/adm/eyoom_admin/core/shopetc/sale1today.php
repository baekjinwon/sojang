...
$sql = " select od_id,
                ...
                (SELECT COUNT(*) FROM g5_download_logs where od_id = 'prime' and SUBSTRING(create_date , 1,10) = '$date') AS primecnt,
                (SELECT COUNT(*) FROM g5_download_logs where od_id != 'prime' and SUBSTRING(create_date , 1,10) = '$date') AS normalcnt
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    $pr = sql_fetch("select od_id from g5_download_logs where od_id = '".$row['od_id']."' ");
    if($pr['od_id']) $list[$i]['chk_nomal'] = "일반";
    else $list[$i]['chk_nomal'] = "기간";

    $list[$i]['query'] = "select od_id from g5_download_logs where od_id = '".$row['od_id']."' ";
