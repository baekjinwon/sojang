...
!function () {
    ...
    db.clients   = [
        <?php for ($i=0; $i<$cnt; $i++) { ?>
        {
            주문일시: "<?php echo $list[$i]['od_date']; ?>",
            실제판매건수: "<?php echo number_format($list[$i]['save']['normalcnt']); ?>",
            다운로드건수: "<?php echo number_format($list[$i]['save']['primecnt']+$list[$i]['save']['normalcnt']); ?>",
            다운로드취소건수 : 0,
            ...
            수수료: "<?php echo number_format($list[$i]['save']['po_point']); ?>",
        },
        <?php } ?>
    ];
}();
